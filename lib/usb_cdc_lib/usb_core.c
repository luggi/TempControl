...
USB_OTG_STS USB_OTG_WritePacket(USB_OTG_CORE_HANDLE * pdev, uint8_t * src, uint8_t ch_ep_num, uint16_t len)
{
    ...
    if (pdev->cfg.dma_enable == 0) {
        ...
        for (i = 0; i < count32b; i++, src += 4) {
            //temp = (uint32_t)(*(src+3) << 24) | (uint32_t)(*(src+2) << 16) | (uint32_t)(*(src+1) << 8) | (uint32_t)(*src);
            //USB_OTG_WRITE_REG32( fifo, temp );
            memcpy((void *) fifo, src, sizeof(uint32_t));
        }
    }
    return status;
}
...
void *USB_OTG_ReadPacket(USB_OTG_CORE_HANDLE * pdev, uint8_t * dest, uint16_t len)
{
    ...
    for (i = 0; i < count32b; i++, dest += 4) {
        ...
        memcpy(dest, (void *) fifo, sizeof(uint32_t));
    }
    return ((void *) dest);
}
