...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define F_CUT 0.1f // 0.1Hz should still be fast enough for the slow temperature changes
static const float fc_temp = 0.5f / (M_PI * F_CUT); 
/* Private macro -------------------------------------------------------------*/
/* Global variables ---------------------------------------------------------*/
extern config_t cfg;
control_t control;
...
int main(void)
{
    ...
    while (1) {
        ...
        if (interval1 < time_now) {
            ...
            if (!cfg.manualMode) {
                pid_calc(&cfg.pid1, control.setpoint[PID1], control.temperature_smooth[SENSOR1], ((float)cfg.cycletime / 1000.0f));
                pid_calc(&cfg.pid2, control.setpoint[PID2], control.temperature_smooth[SENSOR2], ((float)cfg.cycletime / 1000.0f));
            }
...
void get_temperature(void)
{
    ...
    static float previous_temperature[TEMPERATURE_SENSOR_COUNT];
    ...
    for(int i = 0; i < TEMPERATURE_SENSOR_COUNT; i++) {
        ...
        control.temperature_smooth[i] = control.temperature_smooth[i] + (0.01f  / (fc_temp + 0.01f)) * (control.temperature[i] - control.temperature_smooth[i]); //first order low pass filter fc=0.5Hz
    }
}
